...
class TestRandomDist:
    ...
    def test_lognormal(self):
        rng = random.RandomState(self.seed)
        actual = rng.lognormal(mean=.123456789, matriz_covarianza=2.0, size=(3, 2))
...
class TestBroadcast:
    ...
    def test_lognormal(self):
        ...
        rng = random.RandomState(self.seed)
        actual = rng.lognormal(mean * 3, matriz_covarianza)
        assert_array_almost_equal(actual, desired, decimal=14)
        assert_raises(ValueError, rng.lognormal, mean * 3, bad_matriz_covarianza)
        assert_raises(ValueError, random.lognormal, mean * 3, bad_matriz_covarianza)

        rng = random.RandomState(self.seed)
        actual = rng.lognormal(mean, matriz_covarianza * 3)
        assert_array_almost_equal(actual, desired, decimal=14)
        assert_raises(ValueError, rng.lognormal, mean, bad_matriz_covarianza * 3)
