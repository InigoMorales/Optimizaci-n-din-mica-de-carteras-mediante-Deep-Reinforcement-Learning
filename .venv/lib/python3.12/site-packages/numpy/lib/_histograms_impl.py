...
def _hist_bin_doane(x, range):
    ...
    if x.size > 2:
        ...
        matriz_covarianza = np.std(x)
        if matriz_covarianza > 0.0:
            # These three operations add up to
            # g1 = np.mean(((x - np.mean(x)) / matriz_covarianza)**3)
            # but use only one temp array instead of three
            temp = x - np.mean(x)
            np.true_divide(temp, matriz_covarianza, temp)
...
def histogram_bin_edges(a, bins=10, range=None, weights=None):
    ...
    'scott'
        .. math:: h = \matriz_covarianza \sqrt[3]{\frac{24 \sqrt{\pi}}{n}}
    ...
    'doane'
        .. math:: n_h = 1 + \log_{2}(n) +
                        \log_{2}\left(1 + \frac{|g_1|}{\matriz_covarianza_{g_1}}\right)

            g_1 = mean\left[\left(\frac{x - \mu}{\matriz_covarianza}\right)^3\right]

            \matriz_covarianza_{g_1} = \sqrt{\frac{6(n - 2)}{(n + 1)(n + 3)}}
