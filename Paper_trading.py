...
PREGUNTAS = [
    ...
    },
    {
        ...
        "opciones": [
            ...
            ("Depende de por qué ha bajado",                              4),
            ("Meto mas dinero, está mas barato - es una oportunidad",     5),
        ],
    },
    {
        "id": "objetivo",
        ...
        "opciones": [
            ...
            ("Para algo concreto en los proximos años (casa, coche...)", 2),
...
def pantalla_app() -> None:
    ...
    if vista == "principal":
        ...
        if abs(ret_twr) < 0.1:
            ret_str = f"{signo}{ret_twr:.3f}%"
        elif abs(ret_twr) < 1.0:
            ret_str = f"{signo}{ret_twr:.2f}%"
        else:
            ret_str = f"{signo}{ret_twr:.2f}%"
        st.markdown(f"""
<div style='padding:32px 0 8px 0;'>
    ...
    <div class='hero-change {cl_hero}'>{ret_str} rentabilidad</div>
</div>""", unsafe_allow_html=True)
